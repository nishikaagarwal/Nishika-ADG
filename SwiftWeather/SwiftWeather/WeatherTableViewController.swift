...
//  SwiftWeather
//
//  Created by Nishika Agarwal on 19/03/20.
...
class WeatherTableViewController: UITableViewController, UISearchBarDelegate {
        ...
        CLGeocoder().geocodeAddressString(location) { (placemarks:[CLPlacemark]?, error:Error?) in
            if error == nil {
                if let location = placemarks?.first?.location {
                    Weather.forcast(withLocation: location.coordinate, completion: { (results:[Weather]?)
                        in
    ...
    override func tableView(_tableView: UITableView, titleForHeaderInSection: Int -> String? {
